...
function markPointPage (mpp, mppAccess, option){ // closure
	...
	const miMOTIF = 0, miRECURSIVE = 1, miTYPE = 2
		, miINSTP = 3, miFUNCTION = 4, miPARAM = 5, miOPTION = 6
	...
	function decodeInstructions ( tableMarkerSplit ){ //__:decode instruction core, parser bas->haut \\
		...
		var decodeStack = [], topDecodeStack, motifTopEnd = "" 
...
*/
		...
		function decode ( ){ // decode et call selon motif Db embInst.[startDecode|actual|endDecode]
			...
			aData = aDataEncode(nData.substring(embInstDecode[diALL].length))
...
//__:TODO: à contrôler aDataEncode géré ailleur //__...
				if (tableMarkerSplit[tableMarkerSplit.length-1]) tableMarkerSplit[tableMarkerSplit.length-1] += "!" + nData 
				else waitOutData = aDataEncode(nData) + waitOutData
				}
		//______:standard instructions ________/...
			...
			function immediateDataInst (){ // aData collés à embInst jusqu'au premier caractère blanc
	...
	__*/			
			//:\____ closeInst __/:\\ CLSi and CLSAi
			function closeInst (){ //__:avec une instruction de fermeture __/...
				var patchUnstack = ()=>{ return unStackRep = unstack() }
				var unStackRep = topDecodeStack[diEndALT] ? [topDecodeStack] : patchUnstack()
			...
			function endStdInst (){ // instruction de fin embInst 
				...
				motifTopEnd =  ( topDecodeStack = embInstDecode )[diALL] =  embInstDecode[diEndALT] ? "" : (embInstDecode[diTARGET]||"") + (embInstDecode[diPROPERTY]||"")
				waitOutData = ""
				}
			//:\____ sequencedInst __/:\\
			function sequencedInst (){ //__:temporisation et attente identification début de liste pour traitement __/...
				var decoder = actualEmbInst[miFUNCTION]( embInst, aData , unstack)
			...
			//:\____ unstack __/:\\
			function unstack( otherWait ){ 
//if (motifTopEnd === 0){
//	console.log("topEnd ", motifTopEnd, dataEndStack[dataEndStack.length-1])}
				const genericEnd =  ["","",,,,";"]
				var fldActual
				if ( motifTopEnd.length //__:not match ?
					&& ( otherWait && otherWait.substring(0,motifTopEnd.length ) != motifTopEnd 
						|| ( (fldActual = true) && actualEmbInst[miMOTIF].substring(0,motifTopEnd.length ) != motifTopEnd) 
						)
					) { 
					console.warn (motifTopEnd,"trigger end for",embInstDecode.join(""), aData)
					return [ genericEnd, "" , "" ]
					} 
				//__: match consomme une instruction commandEnd (dépile) __/...
				//__:test CLSAi si attente subInst
				if (  !fldActual && topDecodeStack[diEndALT] && actualEmbInst[miTYPE] != CLSAi ) {
					console.warn (motifTopEnd,"trigger end for",embInstDecode.join(""), aData)
					return [ genericEnd, "" , "" ]
					} 
var tmpEndDecode = topDecodeStack
				...
				var rep = dataEndStack.length
					? [ tmpEndDecode, aDataEncode(dataEndStack.pop())+waitOutStack.pop(), fldActual ]
					: [ genericEnd, "", ""]
				   return rep
	...
	function embInstMotifDb ( ){ // motif Db et traitement \\
		...
		//__:init listHtml __/...
		listHtml()
		return [//__:TODO: ajouter match test si besoin
		...
		//  |motifInst		|closed	|instruc|Inst.	|interpreter||param		|option		|
		//  |target property|by		|tion	|Param	|function	||			|			|
		//  |		|		|other	|type	|		|call		||			|			|
			,{ name:"markPointPage", type:"mpp", aDataEncode:toHtmlText, href:"" }
			,{ SplitTxt:"Split text" }
			,[["\""	,"\""	],CBO	,SGLi	,		,hTitle		,"h"		,] // \n title(0) et h(1-6) 
			,[["\""	,"\"."	],CBO	,SGLi	,		,hTitle		,"h"		,] // \n title(0) et h(1-6) centré 
			,[["\""	,"\"="	],CBO	,CLSi	,		,hTitle		,"h"		,EBR] // cover(0) et h(1-6);  mode enterBr
			,[["\""	,"\".="	],CBO	,CLSi	,		,hTitle		,"h"		,EBR] // cover(0) et h(1-6);  centré et mode enterBr
...
//			,[["\"\"#",		],CBO	,CLSi	,		,aHref		,"a"		,true] // a block
			...
			,[["\"..","%"	],		,NDTi	,		,charCode	,"2026"		,] // code caractère point de suspension Unicode
			...
			,[["\"_","%"	],		,NDTi	,		,charCode	,"00A0"		,] // code caractère espace insécable
			,[["\"_",		],		,CLSi	,		,unbreakable,"span"		,] // span unbreakable white-space:nowrap
			,[["\"\"_",		],		,CLSi	,		,unbreakable,"div"		,] // div unbreakable white-space:nowrap
			,[["\"_",'"'	],		,CLSi	,		,unbreakable,"span"		,] // span ecriture collée unbreakable
			,{ FluxTxt:"Flux text" }
...
//			,[["\"-=",		],		,CLSi	,		,block		,"span"		,true] // span block
			...
			,[["\"\"-.",	],		,CLSi	,		,center		,"div"		,true] // div inline-block text-align center
			...
			,[["**"	,		],		,SEQi	,2		,listHtml	,			,] // 
			,[["*"	,		],		,SEQi	,1		,listHtml	,			,] //
			,[["***","="	],		,SEQi	,3		,listHtml	,			,] // block
			,[["**"	,"="	],		,SEQi	,2		,listHtml	,			,] // block 
			,[["*"	,"="	],		,SEQi	,1		,listHtml	,			,] // block
			,[["**#",		],		,SEQi	,2		,listHtml	,			,] // pour th table
			,[["*#"	,		],		,SEQi	,1		,listHtml	,			,] // pour th table
			,[["*."	,		],		,SEQi	,1		,listHtml	,			,] // 
			,[["**_",		],		,SEQi	,-2		,endListHtml,			,] // 
			,[["*_"	,		],		,SEQi	,-1		,endListHtml,			,] //
			,[["**#_",		],		,SEQi	,-2		,endListHtml,			,] // pour th table
			,[["*#_",		],		,SEQi	,-1		,endListHtml,			,] // pour th table
		...
		//:\____ hTitle __/:\\
		function hTitle ( embInst, aData1 ){
			return tagOpenClose( embInst.actual[miPARAM]+embInst.startDecode[diNAME]
				, encodeAttribut( embInst.startDecode[diPROPERTY][1]=="." ? {style:{"text-align":"center"}} :"", embInst )
				, modeBR(aData1, embInst.actual) ) 
			}
		function hrLine ( embInst ){
		...
		function paragraphe ( embInst, aData1 ){
			//__:TODO: accepter \t en premier caractère pour indentation première ligne
			return tagOpenClose( embInst.actual[miPARAM], "", modeBR(aData1, embInst.actual) )
			}
		function br ( embInst ){
		...
		//:\____ commentHtml __/:\\
		function commentHtml ( embInst, aData1 ){
			return "<!-- " + aData1 + " -->"
			}
		function inlineBlock ( embInst, aData1 ){
			return tagOpenClose( embInst.actual[miPARAM]
				, encodeAttribut( embInst.actual[miOPTION]? {style:{display:"inline-block"}} : "", embInst )
				, aData1 )
			}
		function block ( embInst, aData1 ){
			return tagOpenClose( embInst.actual[miPARAM], embInst.actual[miOPTION]?" style='display:block'":"" , aData1 )
			}
		function center ( embInst, aData1 ){
			return tagOpenClose( embInst.actual[miPARAM], embInst.actual[miOPTION]?" style='text-align:center;display:inline-block' ":" style='text-align:center' " , aData1 )
			}
		function charCode ( embInst, aData1 ){ // TODO: à simplifier entrer directement en Unicode car aData est Unicode
			var code = embInst.startDecode[diNAME]
			if (embInst.actual[miPARAM]) return "&#x" + embInst.actual[miPARAM] + ";" + aData1
			if ( /\d+|x[A-Fa-f\d]+/.exec(code) ) code = "#" + code
			return "&" + code + ";" + aData1
			}
		function richText ( embInst, aData, coreUnstack ){
...
//				case "i": tagName = "i"; break;
				...
				case "v:": styleDisplay.attribut.open = "" // remplacé par =open
				case "v": tagName = "details";
					embInst.actual = Array.from( embInst.actual );
					embInst.actual[miPARAM] = ["details","summary"];
					embInst.actual[miOPTION]? "": styleDisplay.style.display = "inline-block";
					return headerSubInst ( embInst, aData, coreUnstack )
		...
		function unbreakable ( embInst, aData1 ){
			return tagOpenClose( embInst.actual[miPARAM],  'style="overflow: visible; white-space: nowrap;"' , aData1 )
			}
		function code ( embInst, aData1 ){ // code avec coloration syntaxique
			...
			return tagOpenClose( embInst.actual[miPARAM], style , aData1 )
			}
		function headerSubInst ( embInst, aData, coreUnstack ){ // instruction avec un header
			var rep = "", style = "", unstack
			if ( embInst.endDecode[diEndALT] == ":" ) {
				rep = tagOpenClose( embInst.actual[miPARAM][1], style , aData )
				unstack = coreUnstack()
				embInst.endDecode = unstack[0]
				aData = unstack[1]
				}
			return tagOpenClose( embInst.actual[miPARAM][0], style , rep + aData )
			}
		function footerSubInst ( embInst, aData, coreUnstack ){ // instruction avec un header
			var rep = "", style = "", unstack
			if ( embInst.endDecode[diEndALT] == "#" ) {
				unstack = coreUnstack()
				embInst.endDecode = unstack[0]
				rep = tagOpenClose( embInst.actual[miPARAM][1], style , unstack[0] )
				}
			return tagOpenClose( embInst.actual[miPARAM][0], style , aData +rep )
			}
...
//				var matchList =  matchListDb[embInstStartName.substring(0,2)] || matchListDb[embInstStartName[0]]
				...
				if (!matchList) return  embInst.actual[miINSTP]//__:un nombre positif non nul pour motifEnd indique élément de liste autre que fin ou début __/...
//				if (!matchList) return  /\**,?/.exec( embInst.startDecode[diTARGET] + embInstStartName )[0].length || "" //__:un nombre positif non nul pour motifEnd indique élément de liste autre que fin ou début __/...
				...
				var startSequence = embInst.actual[miINSTP] //__:initialisation selon méthode d'entrée dans la liste __/... 
...
//_ graphe _________________________________\\
		...
		function endListHtml ( embInst ){ // élément de fin de liste
			return embInst.actual[miINSTP] //__:un nombre négatif pour motifEnd indique fin de liste
			}
...
//			try { decodeAction(embInst.startDecode[diCMD]) } catch (e){}
			...
			tmpStyles.length ? attributs.style = tmpStyles.join( "; " ) : delete attributs.style
			attributs.class.length ? attributs.class = attributs.class.join(' ') : delete attributs.class
			var tmpAttributs = []
			for ( var attribut in attributs) { tmpAttributs.push ( attribut +  (attributs[ attribut ] != ""  ? '="' +  attributs[ attribut ] + '"' : "" )) } 
...
//  |		|		|other	|type	|		|call		||			|			|
	...
	,[["//"	,		],CBO	,CLSAi	,		,comment	,			,] // de bloc
		...
		//__:encode function call __/...
		//:\____ comment __/:\\
		function comment ( embInst, aData, unstack ){
